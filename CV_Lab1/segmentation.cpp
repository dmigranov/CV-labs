...
void split(Region &region, uint iterNum)
{
	...
	if (homogeneity(regmat) > k) //в зависимости от номера итерации использовать разные формулы: на первых п€ти можно по L, далее - CIEDE2000
	{
...
void normalizedCut(Mat &orig, Mat mask, uint iter)
{
	...
	//TODO: D^-1 * (D - W): первый р€д делим на d1 второй на d2 и т.д
	/*for (; it != it_end; it++)
	{
	...
	if (eigs.info() == Spectra::SUCCESSFUL)
	{
		...
		const auto evector = evectors.col(0); //отсортированы так, что сначала идут бќльшие, поэтому нужен нулевой
